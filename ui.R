...
shinyUI(fluidPage(
    ...
    ui<- navbarPage( title="",
                     tabPanel("Avant propos",  p("Grâce aux avancées technologiques, les établissements financiers réduisent les risques encourus à l’aide d’informations sur les clients. 
                             ...
                             p(strong("Ainsi, l’objectif de notre projet est d’évaluer ce score à l’aide des différentes méthodes de classification supervisée,
                             à savoir les méthodes de pénalisation et d’agrégation. Nous allons, par la suite, 
                              ...
                              p(strong("Méthode de pénalisation : ")), 
                              p("En présence de Big Data, les solutions analytiques traditionnelles font face à un problème majeur : les corrélations spécieuses."),
p("Dans le modèle de régression linéaire, l’estimation du vecteur des paramètres par la méthode des MCO requiert que la matrice des variables explicatives soit de plein rang. Or, cette condition est violée en présence des fortes corrélations spécieuses entre variables inhérentes aux données massives."),
p("Ceci a pour conséquence une instabilité des paramètres estimés et donc un impact sur la stabilité des prévisions."),
p("L’estimateur MCO, sous certaines hypothèses, est sans biais et de variance minimale dans la classe des estimateurs linéaires en y. La variance peut néanmoins être très importante, surtout lorsque le nombre de prédicteurs est élevé dans un contexte de Big data."),
p("Ainsi, les méthodes de régression pénalisée se présente comme une solution face à ce problème : s’autoriser un peu de biais pour baisser de manière significative la variance et donc l’erreur de prévision. Elles permettent de réduire la variabilité de l’estimateur MCO via une régularisation des coefficients."),
p("Dans ce projet, nous allons modéliser la probabilité de défaut d’abord par une régression Ridge, qui est historiquement la première méthode de pénalisation proposée, et par la suite, des régressions Lasso et Elastic-Net."), 
                              p(strong("Méthode d’agrégation :")), 
                              p("Les arbres de décisions représentent une suite de règles qui permet de résoudre un problème de classification ou de régression."),
p("Ils sont assez performants pour modéliser de manière non paramétrique des relations non linéaires et permettent de s’adapter à des données complexes."),
p("Ils ont cependant des pouvoirs prédictifs limités et sont souvent sujet au sur- apprentissage. L’algorithme apprend avec beaucoup de précision les données d’entrainement qu’il ne parvient pas à généraliser un résultat satisfaisant sur de nouvelles données."),
p("Les méthodes d’agrégation permettent de pallier à cette insuffisance. Il s’agit d’une technique consistant à entraîner plusieurs arbres pour ensuite considérer l'ensemble de leurs prédictions."),
p("Dans ce projet, nous allons modéliser la probabilité de défaut en utilisant les deux classes de méthodes d’agrégation : le raffinement du Bagging via l’algorithme du Foret aléatoire et les méthodes de Boosting.")
                     ),
                     
                     tabPanel("Les données", 
                              p("On utilise le jeu de données Kaggle :  'Give me some credit'. 
             ...
             credit(1), sinon (0)"),
                     ...
                     tabPanel("Traitement de données", 
            ...
            partionnement du jeu de données."),
             ...
             Il est représentatif des données, est constitué de 75% des observations et est stratifié par le taux de défaut moyen."), 
                             p("Un échantillon de validation sur lequel nous évaluons les capacités prédictives du modèle, 
                               notamment à l’aide des indicateurs de performance. Il est constitué de 25% des observations"),
              ...
              efficace pour la prédiction."),
                                  ...
                                  tabPanel("Ridge", br(),
                                           p("La régression ridge est l'une des méthodes de pénalisation les plus intuitives.
                                             Elle s'utilise pour limiter l'instabilité des prédictions liée à des
                                             variables explicatives trop corrélées entre elles."),
                      ...
                      par rapport à Ridge qui ne définit jamais la valeur du coefficient au zéro absolu."),
                                  ...
                                  tabPanel("Élastic-Net", br(),
                                           p("Pour trouver un compromis entre les deux techniques de pénalisation, 
                                             la régularisation ElasticNet combine les deux approches."),
             ...
             Il est aisément interprétable "),
                              p("Cependant, le Random Forest est le modèle qui possède les meilleurs capacités prédictives, 
                                avec une AUC de 83%."),
             ...
             interprétables peuvent etre utilisées pour expliquer la contribution de chaque 
