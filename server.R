...
shinyServer(function(input, output) {
    ...
    output$ROCSynthese <- renderPlot({
      ...
      newvalid=valid
        ...
        l.fit <- glmnet(Xx, yy, alpha = 0, family = "binomial", lambda =  cv.ridge$lambda.min,standardize = TRUE)
        new <- data.matrix(subset(valid, select=-c(SeriousDlqin2yrs)))
        l2.prob <- predict(l.fit, new, s= cv.ridge$lambda.min, type=c("link"))
        ...
        new <- data.matrix(subset(valid, select=-c(SeriousDlqin2yrs)))
        l3.prob <- predict(l.fit, new, s=cv.Elactic$lambda.min, type=c("link"))
        ...
        lassoROC <- roc(newvalid$SeriousDlqin2yrs ~ as.vector(l.prob),plot=TRUE,legacy.axes=TRUE, lwd=2, 
                       col="red",grid=TRUE, main="Comparaison des courbes de ROC")
        ...
        RidgeROC <- roc(valid$SeriousDlqin2yrs ~ as.vector(l2.prob),plot=TRUE,legacy.axes=TRUE, lwd=2, 
                        col="orange",grid=TRUE, add = TRUE)
        ElasticROC <- roc(valid$SeriousDlqin2yrs ~ as.vector(l3.prob),plot=TRUE,legacy.axes=TRUE, lwd=2, 
